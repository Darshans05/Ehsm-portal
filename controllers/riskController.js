...
exports.getRisks = async (req, res, next) => {
    try {
        console.log(`[${new Date().toISOString()}] GET /api/risks - Querying SAP RISKASSESSMENTSet`);
        ...
        if (Array.isArray(results)) {
            console.log(`[${new Date().toISOString()}] Successfully fetched ${results.length} risks from SAP.`);
        ...
        console.warn(`[${new Date().toISOString()}] SAP Risk OData fetch failed, shifting to local mock data:`, error.message);
